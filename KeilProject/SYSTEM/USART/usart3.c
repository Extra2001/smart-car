...
{  
  ...
  //USART2->BRR=(pclk1*1000000)/(bound);// 波特率设置	 
	...
	UART_DMA_Config(DMA1_Channel2,(u32)&USART3->DR,(u32)USART3_TX_BUF);//DMA1通道7,外设为串口3,存储器为USART3_TX_BUF 
...
{  
	...
	va_end(ap);
	while(DMA_GetCurrDataCounter(DMA1_Channel2)!=0);	//等待通道2传输完成 ,USART2-TX 对应Channel7,USART3-TX对应Channel2 
