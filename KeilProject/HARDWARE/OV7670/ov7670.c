...
void OV7670_AGCAEC(void)
{
    ...
    reg03val = SCCB_RD_Reg(0x03);
    reg03val &= 0x3f;
    reg03val |= 0x00;

    reg04val = SCCB_RD_Reg(0x04);
    reg04val &= 0xfc;
    reg04val |= 0x01;

    SCCB_WR_Reg(0x13, reg13val); //禁止AGC、AWB、AEC

    SCCB_WR_Reg(0x03, reg03val); // VREF[7:6]->AGC[9:8]
    SCCB_WR_Reg(0x00, reg00val); // GAIN[7:0]->AGC[7:0]

    SCCB_WR_Reg(0x07, reg07val); // AECHH[5:0]->AEC[15:10]
    SCCB_WR_Reg(0x10, reg10val); // AECH[7:0]->AEC[9:2]
    SCCB_WR_Reg(0x04, reg04val); // COM1[1:0]->AEC[1:0]

    /* 显示AGC、AEC的实时结果
    reg13val = SCCB_RD_Reg(0x13);
    reg03val = SCCB_RD_Reg(0x03);
    reg00val = SCCB_RD_Reg(0x00);
    reg07val = SCCB_RD_Reg(0x07);
    reg10val = SCCB_RD_Reg(0x10);
    reg04val = SCCB_RD_Reg(0x04);

    printf("13:%02X,03:%02X,00:%02X,07:%02X,10:%02X,04:%02X\r\n",reg13val,reg03val,reg00val,reg07val,reg10val,reg04val);
    */
}
...
void OV7670_Window_Set(u16 sx, u16 sy, u16 width, u16 height)
{
    ...
    // endx=(sx+width*2)%784;	//V*2
    // endy=sy+height*2;
    endx = sx + width * 2; // V*2
    endy = (sy + height * 2) % 784;
