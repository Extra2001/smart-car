...
extern char WF_comm;
unsigned char count_0_1ms = 0; // 0.1ms计数器，作为电机的基本计数器
unsigned char count_5ms = 0;   // 5ms计数器，作为主函数的基本周期
...
void EXTI1_IRQHandler() //中断服务函数，函数定义在启动文件里
{
...
void TIM2_IRQHandler(void)
{
    if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET)
    {
        ...
        count_0_1ms++;
        if (count_0_1ms >= 50) // 5ms
        {
            count_0_1ms = 0;
            count_5ms++;
            tick_5ms = 1;
        }
        if (count_5ms >= 40) // 200ms
        {
            count_5ms = 0;
            tick_200ms = 1;
        }
